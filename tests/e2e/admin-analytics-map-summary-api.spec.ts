...
const MAP_SUMMARY_RATE_LIMIT = 120;
...
test("admin map summary API returns consistent 401/503 error format @admin-analytics-contract", async ({ request }) => {
  const response = await request.get("/api/admin/analytics/map-summary");
...
test("admin map summary API rate-limit returns consistent 429 error format @admin-analytics-contract", async ({ request }) => {
  const response = await hitUntilRateLimited(request, "/api/admin/analytics/map-summary", MAP_SUMMARY_RATE_LIMIT);
