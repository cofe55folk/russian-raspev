...
test("admin analytics summary returns consistent 401/503 error format @admin-analytics-contract", async ({ request }) => {
...
test("admin analytics summary rate-limit returns consistent 429 error format @admin-analytics-contract", async ({ request }) => {
  const response = await hitUntilRateLimited(request, "/api/admin/analytics/summary", SUMMARY_RATE_LIMIT);
