...
## 9.129 Broader route-side pitch matrix теперь подтверждён для трёх последовательных step, включая upper-edge clamp на `12 semitones`

Что оставалось после `9.128`:
1. Двухшаговый repeated-shadow flow уже был доказан для всех route-side surfaces.
2. Но это ещё не закрывало следующий более реалистичный operator pattern:
   - три разных `runPitchShadowPilot(...)` подряд
   - смена знака `pitch`
   - финальный шаг на верхней границе pitch range, где входной запрос должен clamp-нуться к `12`

Что добавлено:
1. В `tests/e2e/appendable-queue-player-pilot.spec.ts` появился новый targeted matrix на normal `/sound/...` route:
   - `tempo=1.04`, `pitch=4`
   - `tempo=0.92`, `pitch=-7`
   - `tempo=1.12`, `pitch=12.8` с ожидаемым persisted/exported result `pitch=12`
2. Эта же matrix теперь отдельно доказана для четырёх route-side surfaces:
   - reload/hydration
...
Почему это важно:
1. Route-side pitch qualification теперь покрывает уже не только “один proof” и не только “последний из двух proof”.
2. Latest-proof semantics подтверждены и для более широкой control-change sequence, где последний шаг идёт в edge-range.
3. Это уменьшает риск перед любыми следующими route-level pitch stress/seek/background сценариями:
   - не нужно снова возвращаться к вопросу, теряется ли последний edge proof при reload/export
...
Проверка:
1. Chromium three-step edge matrix proof — `4/4`
2. WebKit three-step edge matrix proof — `4/4`
3. `npx tsc --noEmit` — pass
...
## 9.131 Hold-aware route-side pitch matrix теперь доказан и для repeated shadow flow с длинными паузами между proof-step

Что оставалось после `9.130`:
1. Мы уже закрыли latest-proof semantics для repeated controls и explicit seeks.
2. Но оставался ещё один практический runtime-вопрос:
   - что будет, если оператор не меняет `tempo/pitch` мгновенно, а между proof-step есть реальные hold-окна по несколько секунд

Что добавлено:
1. В `tests/e2e/appendable-queue-player-pilot.spec.ts` появился новый hold-aware route-level matrix:
   - `runPitchShadowPilot(1.02, 3, 800)`
   - hold `2500ms`
   - `runPitchShadowPilot(0.96, -4, 900)`
   - hold `2500ms`
   - `runPitchShadowPilot(1.10, 6, 900)`
...
Почему это важно:
1. Route-side pitch shadow proof теперь покрывает не только быстрые подряд идущие control changes, но и longer steady-state gaps между ними.
2. Latest committed proof стабильно переживает время удержания playback между шагами, а не только сам факт смены `tempo/pitch`.
3. Это практически добивает дешёвую route-side proof surface перед более дорогими background/interruption сценариями.
...
Проверка:
1. Chromium hold-aware route proof — `4/4`
2. WebKit hold-aware route proof — `4/4`
3. `npx tsc --noEmit` — pass
...
## 9.138 Route-side pause-hidden-resume checkpoint теперь доказан и через report/packet/save surfaces

Что добавлено:
1. Поверх `9.137` добавлено недостающее export/persistence parity для того же interruption-adjacent сценария:
   - direct `downloadReport()`
   - direct `downloadPacket()`
   - `saveCurrentDiagnostics()`
2. Сам route matrix не менялся:
   - `runPitchShadowPilot(1.01, 2, 800)`
   - explicit `pause()`
   - synthetic hidden/pagehide/pageshow cycle
   - explicit `play()`
   - final `runPitchShadowPilot(1.03, 4, 900)`
3. Теперь этот final `1.03 / +4` proof обязан одинаково сохраняться:
   - после reload/hydration
   - в downloaded report
   - в downloaded packet
   - в save-current packet

Почему это важно:
1. Interruption-adjacent qualification больше не зависит только от live reload surface.
2. Persisted report и export artifacts теперь держат тот же latest-proof contract, что и живая route state.
3. Это закрывает последний cheap deterministic gap перед переходом в реально platform-dependent background/interruption behavior.
...
Проверка:
1. Chromium pause-hidden-resume export proof — `4/4`
2. WebKit pause-hidden-resume export proof — `4/4`
3. `npx tsc --noEmit` — pass
